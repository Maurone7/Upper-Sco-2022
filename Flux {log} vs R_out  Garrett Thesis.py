...
for wavelengths in wavelength_list:
    ...
    for inclinations in i_list:
        ...
        # define part of the function 2.1 to be integrated
            def fun(R, h, f, c, k_b, T):
                return planck_function(h,f,c,k_b,T) * R

        # define integration
            def int(fun, h, f, c, k_b, T):
                return quad(fun,
                            R_in, R_out, args=(h, f, c, k_b, T))

            def final(fun, h, f, c, k_b, T, tau):
                return (2 * np.pi * np.cos(i) / (d ** 2)) * int(fun, h, f, c, k_b, T)[0]
