    ...
    for inclinations in i_list:
        ...
        # define tau function
            def tau(sur_den, k_v, i):
                return sur_den(R_in_cm) * k_v / np.cos(i)

        # define planck's function
            def planck_function(h, f, c, k_b, T):
                return 2 * h * (f ** 3) / ((c ** 2) * (np.e ** (h * f / (k_b * T(flaring, L, s_b, R_in_cm))) - 1))
...
# k_v proportional \nu ^ 0.4
# k_v at 230 GHz = 2.3 cm^2/g
# 230 ^ 0.4 / 2.3 * lambda/c ^  0.4
# k_v = 2.3 cm^2/g * (nu / 230 GHz) ^ 0.4