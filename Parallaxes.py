...
for index_ruwe, element_ruwe in enumerate(ruwe_list):
    if float(element_ruwe) > 1.6:
        print(sources_106[index_ruwe])
        print(element_ruwe)
        parallax_106[index_ruwe] = '6.896551724'
        parallax_error_106[index_ruwe] = '0.83594566'

print(parallax_106[sources_106.index('J16020287-2236139')])
...
# create list with luminoisties error NOT in log scale
luminosity_upper_error = []
luminosity_lower_error = []

for x in range(106):
    luminosity_upper_error.append(10**(luminosity_list_barenfeld[x] + luminosity_list_barenfeld_error[x]) - 10**luminosity_list_barenfeld[x])
    luminosity_lower_error.append(10**(luminosity_list_barenfeld[x] - luminosity_list_barenfeld_error[x]) - 10**luminosity_list_barenfeld[x])

for x in range(106):
    luminosity_lower_error_106.append(update_luminosity_error(distances_106[x], distances_lower_error_106[x],
                                                              luminosity_list_barenfeld[x], luminosity_lower_error[x]))
    luminosity_upper_error_106.append(update_luminosity_error(distances_106[x], distances_upper_error_106[x],
                                                              luminosity_list_barenfeld[x], luminosity_upper_error[x]))

luminosity_upper_error_1 = []
luminosity_lower_error_1 = []

#calculate in the error in log scale
for x in range(106):
    luminosity_upper_error_1.append(np.log10(luminosity_106[x] + luminosity_upper_error_106[x]) - np.log10(luminosity_106[x]))
    luminosity_lower_error_1.append(np.log10(luminosity_106[x] + luminosity_lower_error_106[x]) - np.log10(luminosity_106[x]))

#get rid of log scale in updated luminosities values
...
plt.errorbar(luminosity_list_barenfeld, luminosity_106, yerr=(luminosity_upper_error_1, luminosity_lower_error_1), ls='none', c='gray', **kwargs_errobar)
...
print(distances_106)
print(luminosity_list_barenfeld)